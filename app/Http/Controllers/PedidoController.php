    ...
    public function store(Request $request)
{
    ...
    // Pega o produto e verifica o estoque
    $produto = Produto::find($request->produto_id);

    // Verifica se o estoque é suficiente
    if ($produto->estoque < $request->quantidade) {
        // Retorna mensagem de erro se não tiver estoque suficiente
        return back()
            ->with('message', 'Produto com estoque insuficiente para pedido.')
...
{
    // Encontre o pedido que será deletado
    $pedido = Pedido::findOrFail($id);

    // Encontre o produto relacionado ao pedido
    $produto = Produto::find($pedido->produto_id);

    // Restaura o estoque do produto, adicionando a quantidade do pedido deletado
